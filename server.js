...
function getCurrentWeek(user) {
  ...
  const now = new Date();
  const diffWeeks = Math.floor((now - start) / (7 * 86400000));
...
function buildEmail(email, dueTopics, todaysSubjects) {
  const COLORS = { Math:'#0369a1', English:'#9d174d', Science:'#065f46', History:'#7c3aed', default:'#555' };
  const getColor = (s) => COLORS[Object.keys(COLORS).find(k => s.includes(k))] || COLORS.default;
  ...
  return `
    <div style="font-family:-apple-system,sans-serif;max-width:520px;margin:0 auto;background:#fff;border:1px solid #eee;border-radius:12px;overflow:hidden">
      ...
      <div style="padding:24px 28px">
        ...
        <p style="font-size:11px;color:#bbb;margin:16px 0 0;text-align:center">Schedule: Day 1 → 3 → 5 → 7 → 14 → 30 · <a href="${process.env.APP_URL||''}" style="color:#c4410c">Open app</a></p>
      </div>
    </div>`;
}
...
app.post('/api/register', (req, res) => {
  ...
  res.json({
    topics: user.topics,
    timetable: user.timetable || { A: {}, B: {} },
    week: getCurrentWeek(user),
    todaysSubjects: getTodaysSubjects(user)
  });
});
...
// ── Cron: 5pm AEST = 7am UTC ─────────────────────────────────────────────────
cron.schedule('0 7 * * 1-5', async () => {
  console.log('[cron] Running 5pm reminders...');
...
async function runDailyReminders() {
  ...
  for (const [email, user] of Object.entries(db.users)) {
    ...
    try {
      const subj = due.length
        ? `📚 ${due.length} topic${due.length>1?'s':''} to recall today`
        : `📚 ${todaysSubjects.length} classes tracked — recall.`;
...
app.listen(PORT, () => console.log(`✅ recall v2 on port ${PORT}`));
